"""
Получить API Key в Google AI Studio
pip install -U google-generativeai - не поддерживается
...
from envs import OPENROUTER_TOKEN, HF_TOKEN, GEMINI_API_KEY
...
print(f"Gemini library version: {genai.__version__}")
...
class BatchEnglishExtractor:
    def __init__(self, api_key):
        ...
        self.client = genai.Client(api_key=self.api_key) # Setup the Client
      #  self.model_name = "gemini-2.0-flash"
        self.model_name = "gemini-1.5-flash-002" # имеет более высокие лимиты
    ...
    def translate_word(self, word, system_prompt, temperature=0.1):
        ...
        try:
            response = self.client.models.generate_content(
                ...
                contents=f"Word to translate: {word}",
            )
            return response.text
        except Exception as e:
            if "429" in str(e):
                print("🛑 Превышен лимит. Ожидание 60 секунд...")
                sleep(60)
    ...
    def judge_adequacy(self, word, translation, judge_system_prompt):
        ...
        try:
            # Используется тот же клиент Gemini, что и для перевода
            response = self.client.models.generate_content(
                model="gemini-1.5-flash-002",
                config=types.GenerateContentConfig(
                    system_instruction=judge_system_prompt,
                    temperature=0.1,
                    max_output_tokens=2  # Нам нужна только одна цифра
                ),
                contents=prompt,
            )
            ...
            if "429" in str(e):
                print("🛑 Превышен лимит для судьи. Ожидание 60 секунд...")
                sleep(60)
...
if __name__ == "__main__":
    ...
    extractor = BatchEnglishExtractor(api_key=GEMINI_API_KEY)
