...
class BatchEnglishExtractor:
    def __init__(self):
        ...
        if self.use_gpu:
            ...
            print(f"Использую: {torch.cuda.get_device_name(0)}")
        else:
        ...
        model_name = "unsloth/Meta-Llama-3.1-8B-Instruct-bnb-4bit"  # Команда Unsloth делает специальные оптимизированные версии Llama 3.1. Эта модель идентична оригиналу, но доступна
        # model_name = "unsloth/mistral-7b-v0.3-bnb-4bit" #  НЕ требует лицензий и подтверждений

        # # Создаем конфигурацию для 8-битного режима
        # bnb_config = BitsAndBytesConfig(
        #     load_in_8bit=True
        # )
        print(f"--- Загружаю локальную модель: {model_name} ---")
        self.tokenizer = AutoTokenizer.from_pretrained(model_name, token=HF_TOKEN)  # TODO объяснить
    ...
    def translate_word(self, word, system_prompt, temperature=0.1, do_sample=False):
        ...
        # --- ВОТ ТУТ ИСПРАВЛЕНИЕ: берем именно тензор input_ids ---
        input_ids = inputs.input_ids

        # 3. Генерируем (теперь передаем тензор, у которого ЕСТЬ .shape)
        outputs = self.model.generate(
            input_ids,  # передаем тензор, а не весь словарь inputs
            max_new_tokens=800,
        ...
        # 4. Декодируем (срез теперь сработает идеально)
        response = self.tokenizer.decode(outputs[0][input_ids.shape[-1]:], skip_special_tokens=True)
        return response.strip()


    def judge_adequacy(self, word, translation, judge_system_prompt):
        """Модель оценивает работу через Llama-3.1 API локально."""
        try:
            ...
            # 2. Применяем шаблон чата (tokenize=False, чтобы потом токенизировать вручную и иметь доступ к .input_ids)
            text = self.tokenizer.apply_chat_template(messages, tokenize=False, add_generation_prompt=True)

            # 3. Токенизируем и сразу берем .input_ids (это исправляет твою ошибку KeyError: 'shape')
            inputs = self.tokenizer(text, return_tensors="pt").to(self.model.device)
            input_ids = inputs.input_ids

            # 4. Генерируем 2 токена (только цифру)
            outputs = self.model.generate(
                input_ids,
                max_new_tokens=2,
                temperature=0.1,
            ...
            # 5. Декодируем только ответ (срезаем промпт через .shape[-1])
            # Мы берем [0], так как батч из 1 строки, и срезаем длину входа
            score_raw = self.tokenizer.decode(outputs[0][input_ids.shape[-1]:], skip_special_tokens=True)
        ...
        for word in words_to_process:
            try:
                ...
                while attempt < max_attempts and not success:
                    ...
                    final_raw = self.translate_word(word, current_prompt, temperature=curr_temp, do_sample=curr_sample)

                    print(f"--- DEBUG: RAW RESPONSE FOR {word} ---")
                    print(final_raw)
                    print("---------------------------------------")
                ...
                print(f"[✓] {word} добавлено в список")  # Add this to verify in console
            except Exception as e:
                ...
                # Сохраняем
        if not self.rows:
            print("❌ ERROR: self.rows is empty! The loop finished without adding any data.")
            return
        df = pd.DataFrame(self.rows)

        if df.empty:
            print("\n" + "!" * 45)
            print("❌ CRITICAL ERROR: The table is EMPTY.")
            print("Check if the model generated ANY text or if the input file is empty.")
            print("!" * 45)
            return  # This stops the script before the KeyError happens
...
if __name__ == "__main__":
    ...
    extractor.process_all_words(OUTPUT_FILE, EXCEL_FILE, prompt_en_8_examples, judge_prompt_en_3)
